...
class HelpUserModel extends Ardent implements UserInterface 
{
    ...
    public function save( array $rules = array(), array $customMessages = array(), array $options = array(), \Closure $beforeSave = null, \Closure $afterSave = null )
    {
        ...
        if( $this->errors()->isEmpty() && ! $this->confirmed && 
            \Config::get('laravel-helpers::auth.signup_email') == true )
        {
            $user = $this;
...
//                Config::get('laravel-helpers::auth.email_queue_name'),                // Queue name
                ...
                function ($message) use ($user) {
                    $message
                        ->to( $user->email )
                        ->subject( Lang::get('laravel-helpers::auth.email.account_confirmation.subject') );
                }
            );
        }
